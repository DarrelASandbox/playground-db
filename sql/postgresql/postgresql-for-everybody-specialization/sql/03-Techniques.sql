...
ALTER TABLE
  fav DROP COLUMN oops;

ALTER TABLE
  fav
ADD
  COLUMN howmuch INTEGER;
...
VALUES
  (1, 1, 1)
  ON CONFLICT (post_id, account_id) DO
UPDATE
SET
...
UPDATE
SET
  howmuch = fav.howmuch + 1 RETURNING *;
...
UPDATE
  ON post FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();

CREATE TRIGGER set_timestamp BEFORE
UPDATE
  ON fav FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();

CREATE TRIGGER set_timestamp BEFORE
UPDATE
  ON comment FOR EACH ROW EXECUTE PROCEDURE trigger_set_timestamp();
...
FROM '03-Techniques.csv' WITH DELIMITER ',' CSV;
