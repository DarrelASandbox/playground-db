...
FROM
  ...
  JOIN docs_gin AS G ON D.id = G.doc_id
WHERE
  G.keyword = ANY (string_to_array(lower('Meet fun people'), ' '));
...
FROM ( SELECT DISTINCT
  ...
  FROM
    docs AS D,
    unnest(string_to_array(lower(D.doc), ' ')) s(keyword)) AS X;
...
FROM ( SELECT DISTINCT
  ...
  FROM
    docs AS D,
    unnest(string_to_array(lower(D.doc), ' ')) s(keyword)) AS K
  LEFT JOIN docs_stem AS S ON K.keyword = S.word;

-- If the stem is there, use it
SELECT
  id,
  CASE WHEN stem IS NOT NULL THEN
    stem
  ELSE
    keyword
  END AS awesome,
...
FROM ( SELECT DISTINCT
  ...
  FROM
    docs AS D,
    unnest(string_to_array(D.doc, ' ')) s(keyword)) AS K
  LEFT JOIN docs_stem AS S ON K.keyword = S.word;

-- Null Coalescing - return the first non-null in a list
-- x = stem or 'teaching'  # Python
SELECT
  COALESCE(NULL, NULL, 'umsi');

SELECT
  COALESCE('umsi', NULL, 'SQL');
...
SELECT
  COALESCE((
    ...
    FROM docs_stem
    WHERE
      word = lower('teaching')), lower('teaching'));
...
SELECT
  to_tsvector('english', 'More people should learn SQL from UMSI');
...
SELECT
  plainto_tsquery('english', 'SQL Python');

SELECT
  plainto_tsquery('english', 'Teach teaches teaching and the if');

-- A phrase is words that come in order
SELECT
  phraseto_tsquery('english', 'SQL Python');
