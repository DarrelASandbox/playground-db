...
SELECT
  ('{ "type": "Neon", "series": "24 Hours of Lemons", "number": ' || generate_series(1000, 5000) || '}')::jsonb;
...
UPDATE
  jtrack
SET
  body = jsonb_set(body, '{ count }',((body ->> 'count')::int + 1)::text::jsonb)
WHERE
  body ->> 'name' = 'Summer Nights';
...
FROM
  swapi
WHERE
  body ->> 'url' LIKE 'https://swapi.py4e.com/api/people/%'
LIMIT 10;
...
FROM
  swapi
WHERE
  body ->> 'url' LIKE 'https://swapi.py4e.com/api/species/%'
LIMIT 10;
...
SELECT
  substring(body ->> 'url', 'https://swapi.py4e.com/api/([a-z]+)/')
FROM
  swapi
LIMIT 1;

SELECT
  ('{"type": "' || substring(body ->> 'url', 'https://swapi.py4e.com/api/([a-z]+)/') || '" }')
FROM
  swapi
LIMIT 1;
...
SELECT
  body ||('{"type": "' || substring(body ->> 'url', 'https://swapi.py4e.com/api/([a-z]+)/') || '" }')::jsonb
FROM
  swapi
LIMIT 1;
...
UPDATE
  swapi SWT
SET
  body = body ||('{"type": "' || substring(body ->> 'url', 'https://swapi.py4e.com/api/([a-z]+)/') || '" }')::jsonb;
